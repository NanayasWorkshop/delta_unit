...
class DifferentialEvolutionOptimizer:
    def __init__(self, obstacles, original_points, spline_diameter=20):
        ...
        self.max_iterations = 200  # maxiter for DE
        self.population_size = 50  # popsize for DE (actually popsize * dim)
        self.mutation_factor = 0.8  # F parameter
        self.crossover_probability = 0.9  # CR parameter
    ...
    def get_bounds(self):
        """Get optimization bounds for middle points"""
        bounds = []
        
        # For each middle point (excluding first and last)
        for i in range(1, self.num_points - 1):
            original_pos = self.original_points[i]
            
            # Allow exploration within a reasonable range around original position
            search_radius = 150.0  # Can be tuned
            
            for coord in range(3):  # x, y, z
                lower_bound = original_pos[coord] - search_radius
                upper_bound = original_pos[coord] + search_radius
                bounds.append((lower_bound, upper_bound))
        
        return bounds

    def callback_function(self, xk, convergence):
        """Callback function for differential evolution progress"""
        # This is called after each generation
        if hasattr(self, '_last_callback_eval'):
            if self.evaluation_count - self._last_callback_eval > 50:
                print(f"  Generation update: {self.evaluation_count} evaluations, convergence={convergence:.6f}")
                self._last_callback_eval = self.evaluation_count
        else:
            self._last_callback_eval = self.evaluation_count

    def solve(self):
        """Main differential evolution optimization"""
        ...
        # Initial guess (flatten middle points of original)
        initial_vector = self.points_to_vector(self.original_points)
        
        print(f"Optimizing {len(initial_vector)} parameters ({len(initial_vector)//3} points)")
        ...
        try:
            result = differential_evolution(
                ...
                disp=False,  # We handle our own progress display
            ...
            # Extract solution
            final_points = self.vector_to_points(result.x)
            
        except Exception as e:
            print(f"Differential Evolution failed: {e}")
            # Fallback to original points
            final_points = self.original_points.copy()
        ...
        final_score = result.fun if hasattr(result, 'fun') else 1e6
        ...
        optimization_result = {
            ...
            'de_result': result  # Store full DE result for analysis
        }
        
        return optimization_result

def visualize_results(optimizer, result):
    """Visualize the differential evolution result"""
    fig = go.Figure()
    ...
    if result['spline_points'] is not None:
        fig.add_trace(go.Scatter3d(x=result['spline_points'][:, 0], 
                                  ...
                                  name=f"DE Solution (Score: {result['score']:.1f})"))
    ...
    fig.update_layout(
        title=f"Differential Evolution Optimization - {result['method']}",
        scene=dict(
            xaxis_title="X",
            yaxis_title="Y", 
            zaxis_title="Z",
...
if __name__ == "__main__":
    ...
    # Create and run DE optimizer
    optimizer = DifferentialEvolutionOptimizer(obstacles, original_points, spline_diameter=20)
